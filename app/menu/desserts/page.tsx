...
export default function DessertsMenu() {
    const dessertsCategories = [
        { key: 'desserts', data: menuData.desserts.desserts }
    ];
    ...
    return (
        ...
        >
            ...
            >
                ...
                >
                    Explore Our <span className="text-[#7E4300]">Desserts Menu</span>
                </motion.h1>
                <motion.div
            ...
            >
                <motion.div 
                    ...
                    viewport={{ once: true }}
                >
                    {dessertsCategories.map((category, index) => {
                            ...
                            >
                                ...
                                {isEven ? (
                                    ...
                                    <Image 
                                        ...
                                        width={120} 
                                        height={120} 
                                        className={`hidden md:block absolute -left-8 top-1/2 transform -translate-y-1/2 w-16 h-16 lg:w-24 lg:h-24 opacity-20 -z-1 ${index % 4 === 1 ? '-rotate-45' : '-rotate-12'}`}
                                    />
                                )}
                                                    ...
                                                    <h3 className="text-lg md:text-xl font-medium leading-tight mb-1">
                                                        {item.name.split(' ').map((word, i) => {
                                                            if (word.toLowerCase().includes('chocolate') || 
                                                                word.toLowerCase().includes('brownie') || 
                                                                word.toLowerCase().includes('truffle') ||
                                                                word.toLowerCase().includes('cake') ||
                                                                word.toLowerCase().includes('pie') ||
                                                                word.toLowerCase().includes('cream') ||
                                                                word.toLowerCase().includes('sizzle')) {
